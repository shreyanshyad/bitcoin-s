...
case class DataMessageHandler(
  ...
  def handleDataPayloadActual(
    ...
    lazy val resultF = payload match {
      ...
      case HeadersMessage(count, headers) =>
        logger.info(
          s"Received headers message with ${count.toInt} headers from $peer in state $state")
        logger.trace(
                  ...
                  state match {
                    case HeaderSync =>
                      ...
                      if (manager.peers.size > 1) {
                        ...
                        logger.info(s"Only one peer, moving on $peer")
  ...
  private def fetchCompactFilters(
    ...
    logger.info(s"ID ${manager.peers} at fetchCompactFilters.")
    if (
      !syncing ||
      (filterHeaderHeightOpt.isEmpty &&
        filterHeightOpt.isEmpty)
